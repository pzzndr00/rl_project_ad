...
class Duelling_DQN_agent(nn.Module):
    ...
    def __init__(self, input_size, output_size, discount_factor = 0.8, loss_function = nn.MSELoss(), lr = 5e-4,  hidden_size1 = 128, hidden_size2 = 64, hidden_size_3 = 256, adv_type = 'mean',device = 'cpu'):
        ...
        self.fc_adv = nn.Linear(hidden_size2, hidden_size_3)
        self.adv = nn.Linear(hidden_size_3, output_size)

        # value estimation
        self.fc_value = nn.Linear(hidden_size2, hidden_size_3)
        self.value = nn.Linear(hidden_size_3, 1)
    ...
    def forward(self, x):
        ...
        match(self.adv_type):
            ...
            case 'mean':
                out = x_val + x_adv - torch.mean(x_adv, dim = -1, keepdim = True)
